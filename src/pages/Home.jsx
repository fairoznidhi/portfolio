...
import { personalInfo, experiences, projects, skills, competitive, achievements, publications, leadership } from '../data/portfolio';
    ...
    <div ref={pageRef} className={styles.page}>
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.heroInner}>
          ...
          <p className={styles.heroTitle}>{personalInfo.title} <span className={styles.heroAt}>@</span> {personalInfo.company}</p>
          <p className={styles.heroBio}>{personalInfo.bio}</p>
          <div className={styles.heroLinks}>
            <a href={personalInfo.github} target="_blank" rel="noopener noreferrer" className={styles.heroLink}>
              <GithubIcon /> GitHub
            </a>
            <a href={personalInfo.linkedin} target="_blank" rel="noopener noreferrer" className={styles.heroLink}>
              <LinkedinIcon /> LinkedIn
            </a>
            <a href={`mailto:${personalInfo.email}`} className={styles.heroLink}>
              <MailIcon /> Email
            </a>
          ...
          <div className={styles.heroEdu}>
            ...
            <span className={styles.heroEduDivider}>·</span>
            <span className={styles.accent}>CGPA {personalInfo.education.cgpa}</span>
          </div>
        </div>
        ...
        <div className={styles.sectionInner}>
          ...
          <div className={styles.timeline}>
            {experiences.map((exp, i) => (
              <div key={i} data-reveal className={`${styles.reveal} ${styles.timelineItem}`} style={{ animationDelay: `${i * 0.1}s` }}>
                ...
                <div className={styles.timelineContent}>
                  ...
                  <p className={styles.timelineCompany}>{exp.company} <span className={styles.timelineLocation}>· {exp.location}</span></p>
        ...
        <div className={styles.sectionInner}>
          ...
          <div className={styles.skillsGrid}>
            {Object.entries(skills).map(([cat, items], i) => (
              <div key={cat} data-reveal className={`${styles.reveal} ${styles.skillGroup}`} style={{ animationDelay: `${i * 0.08}s` }}>
        ...
        <div className={styles.sectionInner}>
          ...
          <div className={styles.cpGrid}>
            {competitive.map((cp, i) => (
              <a key={cp.platform} href={cp.url} target="_blank" rel="noopener noreferrer"
                data-reveal className={`${styles.reveal} ${styles.cpCard}`}
                style={{ animationDelay: `${i * 0.1}s` }}>
                <div className={styles.cpPlatform}>{cp.platform}</div>
                <div className={styles.cpHandle} style={{ color: cp.color }}>@{cp.handle}</div>
          ...
          {/* Achievements */}
          <div data-reveal className={`${styles.reveal} ${styles.achievementsWrap}`}>
      ...
      <section className={styles.section}>
        <div className={styles.sectionInner}>
          ...
          {publications.map((pub, i) => (
            <div key={i} data-reveal className={`${styles.reveal} ${styles.pubCard}`}>
              <div className={styles.pubVenue}>{pub.venue}</div>
              <h3 className={styles.pubTitle}>{pub.title}</h3>
              {pub.link && pub.link !== '#' && (
                <a href={pub.link} target="_blank" rel="noopener noreferrer" className={styles.pubLink}>Read paper ↗</a>
              )}
            </div>
          ))}
        </div>
      ...
      <section className={styles.section}>
        <div className={styles.sectionInner}>
          ...
          <div className={styles.blogGrid}>
            {blogs.map((b, i) => (
              <Link key={b.id} to={`/blog/${b.id}`} data-reveal
                className={`${styles.reveal} ${styles.blogCard}`}
                style={{ animationDelay: `${i * 0.1}s` }}>
                <div className={styles.blogMeta}>
                  {b.tags.slice(0, 2).map(t => <span key={t} className={styles.blogTag}>{t}</span>)}
      ...
      <section className={styles.section}>
        <div className={styles.sectionInner}>
          ...
          <div className={styles.leadershipGrid}>
            {leadership.map((l, i) => (
              <div key={i} data-reveal className={`${styles.reveal} ${styles.leadershipCard}`}>
                <div className={styles.leadershipRole}>{l.role}</div>
                <div className={styles.leadershipOrg}>{l.org}</div>
                <div className={styles.leadershipMeta}>{l.period} · {l.location}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className={styles.section} id="contact">
        <div className={styles.sectionInner}>
          <div data-reveal className={`${styles.reveal} ${styles.contact}`}>
            ...
            <a href={`mailto:${personalInfo.email}`} className={styles.contactBtn}>
              Say Hello →
            </a>
          </div>
        </div>
...
function ProjectCard({ proj, delay }) {
  return (
    <div data-reveal className={`${styles.reveal} ${styles.projectCard}`} style={{ animationDelay: `${delay}s` }}>
      <div className={styles.projectHeader}>
        ...
        <div className={styles.projectActions}>
          {proj.github && <a href={proj.github} target="_blank" rel="noopener noreferrer" className={styles.projectAction}><GithubIcon /></a>}
          {proj.live && <a href={proj.live} target="_blank" rel="noopener noreferrer" className={styles.projectAction}><ExternalIcon /></a>}
        </div>
      </div>
      <h3 className={styles.projectTitle}>{proj.title}</h3>
      <p className={styles.projectDesc}>{proj.description}</p>
      <div className={styles.projectTags}>
        {proj.tags.map(t => <span key={t} className={styles.projectTag}>{t}</span>)}
      </div>
    </div>
  );
}
